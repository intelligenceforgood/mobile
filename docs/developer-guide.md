...
- One branch per task group, tracked in `planning/change_log.md`.
...
See [`mobile/docs/release-flow.md`](../../../../mobile/docs/release-flow.md) for the placeholder
document with EAS Submit notes.

### 5.5 Configuring the mobile OAuth client (Google Sign-In)

The app uses **Google OAuth 2.0 PKCE + IAP** for production sign-in. The OAuth client must be
created manually in the GCP console. The Executor does not automate this step; follow the
click-path below.

> **Prerequisite:** You must have the Owner or Editor role on the `i4g-dev` GCP project.

1. Open the [GCP Console](https://console.cloud.google.com/) and select project `i4g-dev`.
2. Navigate to **APIs & Services → Credentials**.
3. Click **+ Create Credentials → OAuth client ID**.
4. **Application type:** Choose the target platform:
   - For iOS: select **iOS**. Enter bundle ID `com.intelligenceforgood.i4g`. Leave other fields blank.
   - For Android: select **Android**. Enter package name `com.intelligenceforgood.i4g`. Paste the
     SHA-1 fingerprint from `keytool -list -v -keystore ...` (EAS generates a keystore during the
     first build; run `eas credentials` to retrieve it).
5. Click **Create**. Copy the **Client ID** shown in the success dialog (not the secret — mobile
   clients don't use a secret).
6. Open `mobile/app/.env.dev` and set:
   ```
   EXPO_PUBLIC_GOOGLE_OAUTH_CLIENT_ID=<paste client id here>
   ```
7. The redirect scheme is already set via `EXPO_PUBLIC_OAUTH_REDIRECT_SCHEME` in `.env.dev`. It
   must match the scheme in `app.config.ts` (`scheme` field). Default: `com.intelligenceforgood.i4g`.
8. Rebuild Dev Client (`eas build --profile local --platform ios --local`) so the new scheme is
   embedded in the native binary. Metro restart alone is not enough for scheme changes.

**Verifying the flow:** Sign out in the Settings screen → tap "Sign in with Google" → a browser
sheet opens → authenticate with a `@intelligenceforgood.com` account → the app receives the ID
token, validates it against IAP, and navigates to the Dashboard.
...
| `@sentry/react-native` install warns about ignored build scripts     | pnpm security policy blocks native post-install scripts           | Run `pnpm approve-builds` in `mobile/app/` and approve `@sentry/cli` and `unrs-resolver`. This allows the Sentry source-map upload CLI to be built.  |
| `Sentry.init` called but no events appear in Sentry dashboard        | `EXPO_PUBLIC_SENTRY_DSN` is empty or not set in `.env.dev`        | Copy the DSN from **Sentry → Project → Settings → SDK Setup**. Paste it into `.env.dev`. Full restart Metro.                                         |
| `beforeSend` type error with `@sentry/react-native` v8               | Sentry v8 uses `Event` from `@sentry/types`; import explicitly    | Use `import type { Event } from '@sentry/react-native'` and cast in `beforeSend`.                                                                    |
...
You now have everything you need to start. Review [tdd.md](./tdd.md)
and begin Sprint 0.

## Appendix D · Day-one acceptance test (Definition of Done)

Use this checklist when validating the prototype on a physical device. Corresponds to the DoD in
[`prd_mobile_prototype.md`](../../planning/prd_mobile_prototype.md).

> Copy this checklist into a PR comment or a Notion page and check off each item as you go.

### Authentication

- [ ] App opens and shows the sign-in screen (no auth token stored).
- [ ] "Sign in with Google" opens a browser sheet and completes the OAuth flow.
- [ ] After sign-in, the app navigates to the Dashboard tab.
- [ ] Settings screen shows the correct profile label (e.g., `DEV · direct · google-pkce-iap`).
- [ ] "Sign out" logs out and returns to the sign-in screen.

### Reviews queue

- [ ] Queue tab loads and displays items from the backend.
- [ ] Status and priority filter chips work — tapping "approved" shows only approved reviews.
- [ ] Search box filters results by case ID or review ID.
- [ ] Tapping a row navigates to the case detail screen.
- [ ] After approving/rejecting a case filtered by `new`, the app auto-navigates back to the queue.

### Case detail

- [ ] All sections (Header, Summary, Classification, Timeline, Evidence, Audit Log) render without
      crashing. Sections with no data show an empty state, not a crash.
- [ ] Evidence grid shows thumbnails; tapping a document opens the viewer.
- [ ] Decision sheet opens from the "Decide" button; approve and reject both work.

### Settings

- [ ] Profile label matches `${ENV} · ${apiMode} · ${authProvider}`.
- [ ] App version matches `package.json` `version` field.
- [ ] Sentry toggle is visible; toggling it does not crash.
- [ ] "Switch profile…" button is visible only in `__DEV__` builds.

### Telemetry

- [ ] In a production build (`EXPO_PUBLIC_SENTRY_DSN` set), a test error thrown via
      `throw new Error('test')` appears in the Sentry dashboard with PII fields redacted.

### Stability

- [ ] No crash on cold start.
- [ ] Background → foreground transition does not crash or show a blank screen.
- [ ] No unhandled promise rejections visible in Metro console during normal usage.
