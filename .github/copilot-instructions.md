...
3. **Coding Conventions** – Follow `core/.github/general-coding.instructions.md` for all language-specific standards. Key highlights for this repo: TypeScript uses `camelCase` for variables/functions, `PascalCase` for types/components; Swift and Kotlin follow their platform conventions. Use design tokens from `shared/design-tokens/` for all styling values.
